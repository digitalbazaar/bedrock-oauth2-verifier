...
bedrock.events.on('bedrock.init', async () => {
  ...
  ISSUER_CONFIG_CACHE = new LruCache(issuerConfig.cache);
});
...
export async function checkAccessToken({
  ...
  try {
    ...
    verifyResult = await jwtVerify(jwt, jwks, {
...
async function _getUncachedIssuerConfig({issuerConfigUrl}) {
  ...
  const record = {issuer, jwks, config, next: null, rotating: false};
